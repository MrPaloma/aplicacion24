...
$$.Facturacion.ReporteVentaRsc = class ReporteVentaRsc extends Resource {
    ...
    addBtnBuscar01() {
        ...
        //boton para reporte de ventas por local - vendedor
        $.fn.appButton.get({
            container: `#${this._alias}btn_bus_03`,
            keymnu: this._alias,
            btns: [{keybtn: APP_BTN.SERCH, type: 'button', evts: [{click: 'Obj.Facturacion.ReporteVentaAx.searchMasVendidos'}]}]
        });
    }
    ...
    validaFechas03() {
        if ($(`#${this._alias}txt_desde3`).val().length == 0) {
            Tools.notify().smallMsn({
                content: APP_MSN._031
            });
            return false;
        ...
        if ($(`#${this._alias}txt_hasta3`).val().length == 0) {
            Tools.notify().smallMsn({
                content: APP_MSN._031
            });
            return false;
    ...
    setCajaChica(data) {
        ...
        let efectivo = document.getElementById('tb_efectivo').innerHTML;
        efectivo = efectivo.split(',').join("");
        efectivo = parseFloat(efectivo);
        
        total = efectivo + ingresos - egresos + otrasCajas;
        console.log(total);
        $(`#${this._alias}tb_box_litle`).html(h);
        $(`#tb_otras_cajas`).html(Tools.formatNumber(otrasCajas));
        $(`#tb_ingresos`).html(Tools.formatNumber(ingresos));
        $(`#tb_egresos`).html(Tools.formatNumber(egresos));
        $(`#tb_total_general`).html(Tools.formatNumber(total));
    }
    ...
    setVentasPagado(data) {
        ...
        $.each(data, function (a, b) {
            ...
            tmpEfectivo += b.total_venta;
        });
        $(`#${this._alias}tb_pagado`).html(h);
        $(`#tb_efectivo`).html(Tools.formatNumber(tmpEfectivo));
    }
