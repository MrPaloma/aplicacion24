 ...
 * Descripcion : NotaDebitoAx.js
 * ---------------------------------------
 */
"use strict";

$$.Facturacion.NotaDebitoAx = class NotaDebitoAx extends $$.Facturacion.NotaDebitoRsc {

    constructor() {
        super();
        this._controller = 'notaDebito:';
        this._alias = Exe.getAlias();
        this._dmain = `#${this._alias}${APP_CONTAINER_TABS}`; /*contenedor principal de opcion*/
        this._tour = Obj.Facturacion.NotaDebitoTour;
        this._idGrid = null;
        this._idFormNotaDebito = `#${this._alias}formNotaDebito`;
        ...
        this._grid = (tk) => {
            $(`#${this._alias}grid`).fullgrid({
                ...
                tButtons: [{
                        button: APP_BTN.NEW,
                        event: 'Obj.Facturacion.NotaDebitoAx.formNotaDebito'
                    }],
                ...
                sAxions: {
                    ...
                    group: [{
                            buttons: [{
                                    button: APP_BTN.ESNT,
                                    ajax: {
                                        fn: "Obj.Facturacion.NotaDebitoAx.sendDocSunaht",
                                        serverParams: ["id_nota_debito", "enviado_sunat", "numero_documento"]
                                    }
                                }, {
                                    button: APP_BTN.PDF,
                                    ajax: {
                                        fn: "Obj.Facturacion.NotaDebitoAx.getPDF",
                                        serverParams: ["id_nota_debito", "enviado_sunat"]
                                    }
                                }, {
                                    button: APP_BTN.XML,
                                    ajax: {
                                        fn: "Obj.Facturacion.NotaDebitoAx.getXML",
                                        serverParams: ["id_nota_debito", "enviado_sunat"]
                                    }
                                }, {
                                    button: APP_BTN.CDR,
                                    ajax: {
                                        fn: "Obj.Facturacion.NotaDebitoAx.getCDR",
                                        serverParams: ["id_nota_debito", "enviado_sunat"]
                                    }
                                }, {
                                    button: APP_BTN.ENCL,
                                    ajax: {
                                        fn: "Obj.Facturacion.NotaDebitoAx.sendCliente",
                                        serverParams: ["id_nota_debito", "enviado_sunat"]
                                    }
                                }, {
                                    button: APP_BTN.ANC,
                                    ajax: {
                                        fn: "Obj.Facturacion.NotaDebitoAx.formAnular",
                                        serverParams: ["id_venta", "enviado_sunat", "anulado", "numero_documento", "dias_transcurridos", "dias_habiles", "id_tipo_comprobante", "id_serie", "num_doc"]
                                    }
                                }]
                        }]
                },
        ...
        this._formNotaDebito = (btn, tk) => {
            this.send({
                ...
                response: (data) => {
                    ...
                    this.addButtons();
                    this.getListBoxs(this._idFormNotaDebito);
                    if (Tools.chekMobile()) {
        ...
        this._postNotaDebito = (tk) => {
            this.send({
                flag: 1,
                token: tk,
                element: `#${PREBTNCTXT}${this._alias}${APP_BTN.GNC}`,
                context: this,
                form: this._idFormNotaDebito,
                serverParams: (sData, obj) => {
                ...
                response: (data) => {
                    if (data.rs.ok_error == 'ok') {
                        //enviar a la sunat
                        // this.sendSUNAHT(data);
                        this._sendDocSunaht(null, tk, data.rs.id_nota_debito);
    ...
    formNotaDebito(btn, tk) {
        Tools.addTab({
            context: this,
            id: `${this._alias}-NWV`,
            label: APP_ETIQUET._045,
            fnCallback: () => {
                this._formNotaDebito(btn, tk);
            }
        });
    ...
    getFactura(btn, tk) {
        ...
        this.send({
            ...
            response: (data) => {
                ...
                } else if (data.head.tiene_nota_debito == 1) {
                    Tools.notify().smallMsn({
                        content: APP_MSN._023 + data.head.num_nota_debito
                    });
                } else {
        ...
        this.send({
            ...
            response: (data) => {
                ...
                $(`#${this._alias}a_pdf`).click((e) => {
                    this.getPDF($(e.currentTarget), rs.id_nota_debito, null, tk);
                });
