...
* Descripcion : NotaDebitoModel.php
* ---------------------------------------
*/ 

namespace Facturacion\NotaDebito\Models;
  
class NotaDebitoModel extends \Vendor\DataBase {
    ...
    protected function spGrid() {
        $query = "CALL sp_facturacion_notadebito_grid (:idRol,:local,:iDisplayStart,:iDisplayLength,:pOrder,:pFilterCols,:sExport,:user);";
        $parms = [
            ":idRol" => @$this->_idRol,
            ":local" => @$this->_idLocal,
    ...
    protected function spMantenimientoND() {
        $query = "CALL sp_facturacion_notadebito_mantenimiento ("
                . ":clienteApp,"
                . ":flag,"
                . ":key,"
                . ":serie_nc,"
        ...
        $parms = [
            ...
            ':tipo_nota_debito' => @$this->_form->lst_tipo_nota_debito,
    ...
    protected function qHeadDocumentoFactura() {
        ...
        SELECT 
                ...
                c.ruc,
                v.anulado,
                (SELECT COUNT(*) FROM fac_nota_debito nc WHERE nc.id_venta = v.id_venta) tiene_nota_debito,
                (
			SELECT 
				CONCAT(g.serie,'-',nc.numero_documento) 
			FROM fac_nota_debito nc 
    ...
    protected function qHeadDocumento($id) {
        ...
        SELECT 
                ...
                nc.id_tipo_nota_debito,
                tm.codigo tipo_moneda,
                nc.id_tipo_nota_debito,
                nc.hash_cpe,
                v.fecha_emision fecha_doc_afec,
                tnc.tipo_nota_debito,
        ...
        INNER JOIN fac_tipo_nota_debito tnc ON tnc.id_tipo_nota_debito = nc.id_tipo_nota_debito
        WHERE nc.id_nota_debito = :key
        ";
        $parms = [
            ':key' => $id
        ];
    ...
    protected function qDetailDocumento($id) {
        ...
        SELECT
                ...
                vd.id_tipo_igv,
                0 descuento
        FROM fac_nota_debito_detalle v
        INNER JOIN fac_nota_debito vv ON vv.id_nota_debito = v.id_nota_debito
        INNER JOIN fac_serie er ON er.id_serie = vv.id_serie
        INNER JOIN fac_local_producto l ON l.id_local_producto = v.id_local_producto
        INNER JOIN fac_catalogo c ON c.id_catalogo = l.id_catalogo
        INNER JOIN fac_unidad_medida u ON u.id_unidad_medida = c.id_unidad_medida
        INNER JOIN fac_venta_detalle vd ON vd.id_venta_detalle = v.id_venta_detalle
        WHERE vv.id_nota_debito = :key
        ";
        $parms = [
            ':key' => $id
        ];
    ...
    protected function qRespuestaSunat() {
        ...
        SELECT 
                ...
                v.name_file_sunat,
                CONCAT(t.simbolo,' ',v.total_nota_debito) total_venta,
                w.id_tipo_comprobante,
                v.name_file_sunat,
                c.ruc
        FROM fac_nota_debito v
