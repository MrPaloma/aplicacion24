...
* Descripcion : NotaDebitoController.php
* ---------------------------------------
*/ 

namespace Facturacion\NotaDebito\Controllers;
   
use \Vendor\Controller;
use \Facturacion\NotaDebito\Filters\NotaDebitoFilter;
use \Dompdf\Dompdf;

class NotaDebitoController extends \Facturacion\NotaDebito\Models\NotaDebitoModel {
    
    use Controller {
        Controller::__construct as private __cConstruct;
    }
    use NotaDebitoFilter {
        NotaDebitoFilter::__construct as private __fConstruct;
    }
    
    public function __construct() {
        parent::__construct();  /* constructor del NotaDebitoModel */
        $this->__cConstruct();  /* constructor del Controller */
        $this->__fConstruct();  /* constructor del NotaDebitoFilter */
    }
    ...
    public function postNotaDebito() {
        $rs = $this->spMantenimientoND();
        $data = [
            'rs' => $rs,
            'company' => $this->qCompany(),
            'head' => $this->qHeadDocumento($rs['id_nota_debito']),
            'detail' => $this->qDetailDocumento($rs['id_nota_debito'])
        ];
        echo json_encode($data);
    }
    ...
    public function sendDocSunaht() {
        ...
        $ws = ROOT_WS_SUNAT . "notadebito.php";
        ...
        foreach ($detail as $value) {
            ...
            $items[] = [
                ...
                "txtIGV" => number_format(($value['total'] * $value['igv']), 2,'.',''),
                "txtISC" => "0",
        ...
        $data = [
            ...
            "cod_tipo_motivo" => $head['id_tipo_nota_debito'],
        ...
        $result = [
            ...
            'ruc' => $company['ruc'],
            'total_venta' => $head['total_nota_debito'],
            'id_nota_debito' => $head['id_nota_debito'],
    ...
    public function getPDF() {
        ...
        /* RUC | TIPO DE DOCUMENTO | SERIE | NUMERO | MTO TOTAL IGV | MTO TOTAL DEL COMPROBANTE | FECHA DE EMISION |TIPO DE DOCUMENTO ADQUIRENTE | NUMERO DE DOCUMENTO ADQUIRENTE | */
        $text_qr = $company['ruc'] . '|' . $head['tipo_comprobante'] . '|' . $head['serie'] . '|' . $head['numero_documento'] . '|' . $head['total_igv'] . '|' . $head['total_nota_debito'] . '|' . $head['fecha_emision'] . '|' . $head['id_tipo_documento_identidad'] . '|' . $head['documento_identidad'] . '|';
        $ruta_qr = BASE_URL . 'files/temp/QRND_' . $this->_form->_key . '.png';
        $qr = ROOT . 'files' . DS . 'temp' . DS . 'QRND_' . $this->_form->_key . '.png';
        $ruta_qr = $qr;
        require_once ROOT . 'libs' . DS . 'phpqrcode' . DS . 'qrlib.php';
        ...
        $DomPDF = new DOMPDF();
        $DomPDF->set_option('enable_remote', TRUE);
        $file = ROOT . "files" . DS . "temp" . DS . "tmpND.pdf";
        Obj()->Vendor->Tools->deleteFile($file);
        ...
        $html = file_get_contents(BASE_URL . 'files/template/comprobante/notaDebito.html');
        ...
        $html = str_replace("{TIPO_MOTIVO}", $head['tipo_nota_debito'], $html);
        
        $html = str_replace("{DETAIL}", $productos, $html);
        $html = str_replace("{MONTO_LETRAS}", Obj()->Libs->NumberToString->convert($head['total_nota_debito']), $html);
        $html = str_replace("{SUB_TOTAL}", $head['total_gravada'], $html);
        $html = str_replace("{TOTAL_IGV}", $head['total_igv'], $html);
        $html = str_replace("{TOTAL}", $head['total_nota_debito'], $html);
        $html = str_replace("{LINK}", 'https://bit.ly/2HiRWZI', $html);
        $html = str_replace("{HASH}", $hash, $html);
        $html = str_replace("{QR}", $ruta_qr, $html);
        ...
        if (is_readable($file)) {
            echo json_encode(['result' => 1, 'file' => BASE_URL . 'files/temp/tmpND.pdf']);
        } else {
            echo json_encode(['result' => 2]);
        }
    ...
    private function _sendMailCliente($data) {
        $pdf = BASE_URL . 'files/temp/tmpND.pdf';
